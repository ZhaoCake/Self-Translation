...
2.  **选择目标语言**：在"目标语言"下拉框中选择翻译目标（简体中文 / 繁體中文 / English / 日本語）。
3.  在 **API Key** 输入框中，粘贴您刚才复制的 DeepSeek Key。
4.  点击底部的 **"保存配置"** 按钮（如果需要高速翻译可以选择更大的并行数）。
    *   *看到 "✨ 设置已保存" 提示即表示配置成功。*

> **💡 提示**：如需使用思考（推理）模式，可在"模型名称"中填写 `deepseek-reasoner`（即 DeepSeek V4 Flash 的思考模式别名）。
